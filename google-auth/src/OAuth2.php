 ...
 */
class OAuth2 implements \ahrefs\AhrefsSeoKit\Google\Auth\FetchAuthTokenInterface
{
    ...
    public function generateCredentialsRequest()
    {
        ...
        return new \GuzzleHttp\Psr7\Request('POST', $uri, $headers, \GuzzleHttp\Psr7\build_query($params));
    }
    /**
    ...
    public function buildFullAuthorizationUri(array $config = [])
    {
        ...
        $existingParams = \GuzzleHttp\Psr7\parse_query($result->getQuery());
        $result = $result->withQuery(\GuzzleHttp\Psr7\build_query(\array_merge($existingParams, $params)));
    ...
    {
        if (\class_exists('Firebase\\JWT\\JWT')) {
            return \Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
