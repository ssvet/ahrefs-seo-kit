...
class GCECache
{
     ...
     */
    public function __construct(array $cacheConfig = null, \Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->cache = $cache;
    ...
    public function onGce(callable $httpHandler = null)
    {
        if (\is_null($this->cache)) {
            return \ahrefs\AhrefsSeoKit\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
        }
        $cacheKey = self::GCE_CACHE_KEY;
        $onGce = $this->getCachedValue($cacheKey);
        if (\is_null($onGce)) {
            $onGce = \ahrefs\AhrefsSeoKit\Google\Auth\Credentials\GCECredentials::onGce($httpHandler);
