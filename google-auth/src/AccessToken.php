...
class AccessToken
{
     ...
     */
    public function __construct(callable $httpHandler = null, \Psr\Cache\CacheItemPoolInterface $cache = null)
    {
        $this->httpHandler = $httpHandler ?: \ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        $this->cache = $cache ?: new \ahrefs\AhrefsSeoKit\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // simplejwt
        } catch (\ahrefs\AhrefsSeoKit\Google\Auth\DomainException $e) {
    ...
    private function verifyEs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            $jwkset->add(\SimpleJWT\Keys\KeyFactory::create($cert, 'php'));
        }
    ...
    private function verifyRs256($token, array $certs, $audience = null, $issuer = null)
    {
        ...
        foreach ($certs as $cert) {
            ...
            $rsa->loadKey(['n' => new \phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['n']]), 256), 'e' => new \phpseclib\Math\BigInteger($this->callJwtStatic('urlsafeB64Decode', [$cert['e']]), 256)]);
    ...
    public function revoke($token, array $options = [])
    {
        ...
        $body = \GuzzleHttp\Psr7\stream_for(\http_build_query(['token' => $token]));
        $request = new \GuzzleHttp\Psr7\Request('POST', self::OAUTH2_REVOKE_URI, ['Cache-Control' => 'no-store', 'Content-Type' => 'application/x-www-form-urlencoded'], $body);
    ...
    private function retrieveCertsFromLocation($url, array $options = [])
    {
        ...
        $httpHandler = $this->httpHandler;
        $response = $httpHandler(new \GuzzleHttp\Psr7\Request('GET', $url), $options);
    ...
    {
        $class = \class_exists('Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
