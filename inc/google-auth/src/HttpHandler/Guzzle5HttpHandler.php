...
namespace ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler;
...
class Guzzle5HttpHandler
{
     ...
     * @return Promise
     */
    public function async(\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \GuzzleHttp\Promise\Promise(function () use($futureResponse) {
            try {
                $futureResponse->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\GuzzleHttp\Message\ResponseInterface $response) {
    ...
    private function createGuzzle5Request(\Psr\Http\Message\RequestInterface $request, array $options)
    {
    ...
    private function createPsr7Response(\GuzzleHttp\Message\ResponseInterface $response)
    {
        return new \GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders() ?: [], $response->getBody(), $response->getProtocolVersion(), $response->getReasonPhrase());
    }
}
