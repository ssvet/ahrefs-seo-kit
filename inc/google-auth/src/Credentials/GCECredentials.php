...
namespace ahrefs\AhrefsSeoKit\Google\Auth\Credentials;

use ahrefs\AhrefsSeoKit\Google\Auth\CredentialsLoader;
use ahrefs\AhrefsSeoKit\Google\Auth\GetQuotaProjectInterface;
...
use ahrefs\AhrefsSeoKit\Google\Auth\Iam;
use ahrefs\AhrefsSeoKit\Google\Auth\ProjectIdProviderInterface;
use ahrefs\AhrefsSeoKit\Google\Auth\SignBlobInterface;
 ...
 */
class GCECredentials extends \ahrefs\AhrefsSeoKit\Google\Auth\CredentialsLoader implements \ahrefs\AhrefsSeoKit\Google\Auth\SignBlobInterface, \ahrefs\AhrefsSeoKit\Google\Auth\ProjectIdProviderInterface, \ahrefs\AhrefsSeoKit\Google\Auth\GetQuotaProjectInterface
{
    // phpcs:disable
     ...
     */
    public function __construct(\ahrefs\AhrefsSeoKit\Google\Auth\Iam $iam = null, $scope = null, $targetAudience = null, $quotaProject = null, $serviceAccountIdentity = null)
    {
        $this->iam = $iam;
    ...
    {
        $httpHandler = $httpHandler ?: \ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        for ($i = 1; $i <= self::MAX_COMPUTE_PING_TRIES; $i++) {
            try {
                ...
                // the metadata resolution was particularly slow. The latter case is
                // "unlikely".
                $resp = $httpHandler(new \GuzzleHttp\Psr7\Request('GET', $checkUri, [self::FLAVOR_HEADER => 'Google']), ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]);
    ...
    {
        $httpHandler = \ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpHandlerFactory::build(\ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpClientCache::getHttpClient());
        ...
        $signer = $this->iam ?: new \ahrefs\AhrefsSeoKit\Google\Auth\Iam($httpHandler);
    ...
    {
        $resp = $httpHandler(new \GuzzleHttp\Psr7\Request('GET', $uri, [self::FLAVOR_HEADER => 'Google']));
