...
namespace ahrefs\AhrefsSeoKit\Google\Service;

use ahrefs\AhrefsSeoKit\Google\Model;
use ahrefs\AhrefsSeoKit\Google\Http\MediaFileUpload;
use ahrefs\AhrefsSeoKit\Google\Exception as GoogleException;
use ahrefs\AhrefsSeoKit\Google\Utils\UriTemplate;
...
class Resource
{
    ...
    public function call($name, $arguments, $expectedClass = null)
    {
        if (!isset($this->methods[$name])) {
            ...
            throw new \ahrefs\AhrefsSeoKit\Google\Exception("Unknown function: " . "{$this->serviceName}->{$this->resourceName}->{$name}()");
        }
        ...
        if (isset($parameters['postBody'])) {
            if ($parameters['postBody'] instanceof \ahrefs\AhrefsSeoKit\Google\Model) {
        ...
        foreach ($parameters as $key => $val) {
            if ($key != 'postBody' && !isset($method['parameters'][$key])) {
                ...
                throw new \ahrefs\AhrefsSeoKit\Google\Exception("({$name}) unknown parameter: '{$key}'");
            }
        }
        foreach ($method['parameters'] as $paramName => $paramSpec) {
            if (isset($paramSpec['required']) && $paramSpec['required'] && !isset($parameters[$paramName])) {
                ...
                throw new \ahrefs\AhrefsSeoKit\Google\Exception("({$name}) missing required param: '{$paramName}'");
            }
        ...
        $request = new \GuzzleHttp\Psr7\Request($method['httpMethod'], $url, ['content-type' => 'application/json'], $postBody ? \json_encode($postBody) : '');
        // support uploads
        if (isset($parameters['data'])) {
            $mimeType = isset($parameters['mimeType']) ? $parameters['mimeType']['value'] : 'application/octet-stream';
            $data = $parameters['data']['value'];
            $upload = new \ahrefs\AhrefsSeoKit\Google\Http\MediaFileUpload($this->client, $request, $mimeType, $data);
    ...
    public function createRequestUri($restPath, $params)
    {
        ...
        if (\count($uriTemplateVars)) {
            $uriTemplateParser = new \ahrefs\AhrefsSeoKit\Google\Utils\UriTemplate();
