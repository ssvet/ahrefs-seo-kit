...
namespace ahrefs\AhrefsSeoKit\Google;

use ahrefs\AhrefsSeoKit\Google\AccessToken\Revoke;
use ahrefs\AhrefsSeoKit\Google\AccessToken\Verify;
use ahrefs\AhrefsSeoKit\Google\Auth\ApplicationDefaultCredentials;
use ahrefs\AhrefsSeoKit\Google\Auth\Cache\MemoryCacheItemPool;
...
use ahrefs\AhrefsSeoKit\Google\Auth\OAuth2;
use ahrefs\AhrefsSeoKit\Google\Auth\Credentials\ServiceAccountCredentials;
use ahrefs\AhrefsSeoKit\Google\Auth\Credentials\UserRefreshCredentials;
use ahrefs\AhrefsSeoKit\Google\AuthHandler\AuthHandlerFactory;
use ahrefs\AhrefsSeoKit\Google\Http\REST;
...
class Client
{
    ...
    public function fetchAccessTokenWithAuthCode($code)
    {
        ...
        $httpHandler = \ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpHandlerFactory::build($this->getHttpClient());
     ...
     * @return array access token
     */
    public function fetchAccessTokenWithAssertion(\GuzzleHttp\ClientInterface $authHttp = null)
    {
        ...
        $httpHandler = \ahrefs\AhrefsSeoKit\Google\Auth\HttpHandler\HttpHandlerFactory::build($authHttp);
     ...
     */
    public function authorize(\GuzzleHttp\ClientInterface $http = null)
    {
        $credentials = null;
        $token = null;
        $scopes = null;
    ...
    {
        $tokenRevoker = new \ahrefs\AhrefsSeoKit\Google\AccessToken\Revoke($this->getHttpClient());
    ...
    {
        $tokenVerifier = new \ahrefs\AhrefsSeoKit\Google\AccessToken\Verify($this->getHttpClient(), $this->getCache(), $this->config['jwt']);
        if (null === $idToken) {
    ...
    public function execute(\Psr\Http\Message\RequestInterface $request, $expectedClass = null)
    {
        ...
        $http = $this->authorize();
        return \ahrefs\AhrefsSeoKit\Google\Http\REST::execute($http, $request, $expectedClass, $this->config['retry'], $this->config['retry_map']);
    }
    /**
    ...
    {
        $auth = new \ahrefs\AhrefsSeoKit\Google\Auth\OAuth2(['clientId' => $this->getClientId(), 'clientSecret' => $this->getClientSecret(), 'authorizationUri' => self::OAUTH2_AUTH_URL, 'tokenCredentialUri' => self::OAUTH2_TOKEN_URI, 'redirectUri' => $this->getRedirectUri(), 'issuer' => $this->config['client_id'], 'signingKey' => $this->config['signing_key'], 'signingAlgorithm' => $this->config['signing_algorithm']]);
        return $auth;
    }
    /**
    ...
    protected function createDefaultLogger()
    {
        ...
        if ($this->isAppEngine()) {
            $handler = new \Monolog\Handler\SyslogHandler('app', \LOG_USER, \Monolog\Logger::NOTICE);
        } else {
            $handler = new \Monolog\Handler\StreamHandler('php://stderr', \Monolog\Logger::NOTICE);
        }
    ...
    {
        return new \ahrefs\AhrefsSeoKit\Google\Auth\Cache\MemoryCacheItemPool();
    }
    /**
    ...
    protected function createDefaultHttpClient()
    {
        ...
        if (\defined('\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
            ...
            $guzzleVersion = (int) \substr(\GuzzleHttp\ClientInterface::VERSION, 0, 1);
        }
        if (5 === $guzzleVersion) {
            $options = ['base_url' => $this->config['base_path'], 'defaults' => ['exceptions' => \false]];
            if ($this->isAppEngine()) {
                // set StreamHandler on AppEngine by default
                $options['handler'] = new \GuzzleHttp\Ring\Client\StreamHandler();
    ...
    private function createApplicationDefaultCredentials()
    {
        ...
        if ($signingKey) {
            ...
            $credentials = \ahrefs\AhrefsSeoKit\Google\Auth\CredentialsLoader::makeCredentials($scopes, $serviceAccountCredentials);
        } else {
            ...
            $credentials = \ahrefs\AhrefsSeoKit\Google\Auth\ApplicationDefaultCredentials::getCredentials($scopes, null, $sub ? null : $this->config['cache_config'], $sub ? null : $this->getCache(), $this->config['quota_project']);
        }
        // for service account domain-wide authority (impersonating a user)
        // @see https://developers.google.com/identity/protocols/OAuth2ServiceAccount
        if ($sub) {
            if (!$credentials instanceof \ahrefs\AhrefsSeoKit\Google\Auth\Credentials\ServiceAccountCredentials) {
        ...
        if (!$credentials instanceof \ahrefs\AhrefsSeoKit\Google\Auth\FetchAuthTokenCache) {
            $credentials = new \ahrefs\AhrefsSeoKit\Google\Auth\FetchAuthTokenCache($credentials, $this->config['cache_config'], $this->getCache());
        }
        return $credentials;
    }
    protected function getAuthHandler()
    {
        ...
        return \ahrefs\AhrefsSeoKit\Google\AuthHandler\AuthHandlerFactory::build($this->getCache(), $this->config['cache_config']);
    }
    private function createUserRefreshCredentials($scope, $refreshToken)
    {
        ...
        return new \ahrefs\AhrefsSeoKit\Google\Auth\Credentials\UserRefreshCredentials($scope, $creds);
    }
}
