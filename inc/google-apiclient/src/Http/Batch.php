...
class Batch
{
    ...
    private $rootUrl;
    private $batchPath;
    public function __construct(\ahrefs\AhrefsSeoKit\Google\Client $client, $boundary = \false, $rootUrl = null, $batchPath = null)
    {
        $this->client = $client;
        ...
        if (!empty($body)) {
            ...
            foreach ($parts as $i => $part) {
                ...
                if (!empty($part)) {
                    ...
                    $response = new \GuzzleHttp\Psr7\Response($status, $partHeaders, \GuzzleHttp\Psr7\stream_for($partBody));
                    // Need content id.
                    $key = $headers['content-id'];
                    try {
                        $response = \ahrefs\AhrefsSeoKit\Google\Http\REST::decodeHttpResponse($response, $requests[$i - 1]);
                    } catch (\ahrefs\AhrefsSeoKit\Google\Service\Exception $e) {
